...
- 如果需要本地生成词典或构建自己的 release 包，需要 ECDICT 的 CSV 数据。
...
2. 解压到一个有写入权限的文件夹。
3. 运行 `run-hudict.bat` 或 `HUDict.exe`。
4. 保持 `config.ini` 和 `dictionary.pkl` 与 `HUDict.exe` 在同一个文件夹。

默认 release 脚本构建出的压缩包会包含可直接使用的 ECDICT 词典。
...
维护者可以在本地生成 Windows release 压缩包：

```powershell
.\scripts\build_release.ps1
```

它会生成 `dist\HUDict-vX.Y.Z-windows-x64.zip`。压缩包包含应用、`config.ini`、`dictionary.pkl`、启动脚本和中英文 README。

如果要发布到 GitHub Releases，先安装 [GitHub CLI](https://cli.github.com/) 并登录，然后运行：
...
如果想手动上传，也可以在 GitHub 上为 `vX.Y.Z` 创建 release，并上传 `dist/` 里的 zip。
